...
class SrcLinkInfo:
    ...
    def __repr__(self):
        return f'[[{self.maxLinkCnt}->{self.maxLinkNode.name}--' + \
...
def processNodes(srcNodeMap, nodeGraph, nodeTree, destNode,
                 srcNodes, depth, arrangeType, maxColNodes):
    if len(nodeGraph) == depth:
        nodeGraph.append([])
    nodeColumn = nodeGraph[depth]
    for node in srcNodes:
        if maxColNodes > 0:
            while (maxColNodes == len(nodeGraph[depth]) and
                   len(nodeGraph) > depth):
                depth += 1
        ...
        if depth == 0:
            ...
            if srcLinkInfo is None:  # Already processed
                ...
                if (colNo is None or
                   (colNo < depth and arrangeType == 'LAST') or
                   (colNo > depth and arrangeType == 'FIRST')):
        ...
        processNodes(srcNodeMap, nodeGraph, nodeTree, node, srcNodes,
                     depth + 1, arrangeType, maxColNodes)
...
def displayNodes(nodeGraph, vAlign, xOffset, yOffset):
    ...
    for origCol in nodeGraph:
        ...
        if vAlign in {'MIDDLE', 'BOTTOM'}:
            colHeight = sum([n.dimensions[1] for n in col
                             if n is not None]) + yOffset * (len(col) - 1)
        ...
        for i, node in enumerate(col):
            loc = [currLoc[0] - (maxWidth + node.dimensions[0]) / 2,
                   currLoc[1]]
            if i > 0:
                prevNode = col[i-1]
                loc[1] = prevNode.location[1] - prevNode.dimensions[1] - \
                    yOffset
            node.location = loc
            if node.type == 'GROUP':
                groupNodes.add(node)
...
def displayTree(nodeTree, vAlign, xOffset, yOffset,
                includeGroup, arrangeType, maxColNodes):
    srcNodeMap = createSrcNodeMap(nodeTree)
    # print(srcNodeMap)
    nodeGraph = []
    nodes = nodeTree.nodes
    srcNodes = {n for n in nodes
                if (n in {k.from_node for k in nodeTree.links})}
    pureDestNodes = [n for n in nodes if n not in srcNodes]
    processNodes(srcNodeMap, nodeGraph, nodeTree, None, pureDestNodes, 0,
                 arrangeType, maxColNodes)
    groupNodes = displayNodes(nodeGraph, vAlign, xOffset, yOffset)
    if includeGroup:
        for node in groupNodes:
            nodeTree.nodes.active = node
            bpy.ops.node.group_edit(getOverride(), exit=False)
            bpy.ops.wm.redraw_timer(type='DRAW_WIN_SWAP', iterations=1)
            childNodeTree = node.node_tree
            displayTree(childNodeTree, vAlign, xOffset, yOffset,
                        includeGroup, arrangeType, maxColNodes)
            bpy.ops.node.group_edit(getOverride(), exit=True)
            bpy.ops.wm.redraw_timer(type='DRAW_WIN_SWAP', iterations=1)

def main(context, vAlign, xOffset, yOffset, includeGroup,
         arrangeType, maxColNodes):
    nodeTree = getActiveNodeTree(context)
    displayTree(nodeTree, vAlign, xOffset, yOffset, includeGroup,
                arrangeType, maxColNodes)
...
class LineupNodesParams(PropertyGroup):
    vAlign: EnumProperty(
        ...
        items=(('TOP', 'Top', 'Align top level of nodes'),
    ...
    arrangeType: EnumProperty(
        ...
        items=(('FIRST', 'First', 'Put at first link column'),
               ('LAST', 'Last', 'Put at last link column'),
               ('MAX', 'Max', 'Put where links are maximum')),
        default='LAST'
    )
    xOffset: IntProperty(name='Width Separation', default=50,
...
class LineupNodesOp(Operator):
    ...
    def execute(self, context):
        params = bpy.context.window_manager.lineupNodeParams
        main(context, params.vAlign, params.xOffset, params.yOffset,
             params.includeGroup, params.arrangeType, params.maxColNodes)
        return {'FINISHED'}
...
def register():
    ...
    bpy.types.WindowManager.lineupNodeParams = \
        bpy.props.PointerProperty(type=LineupNodesParams)
